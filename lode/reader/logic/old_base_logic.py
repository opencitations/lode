...
class BaseLogic(ABC):
    ...
    def populate_instance(self, instance, uri: Node):
        ...
        Nella provenance traccia SOLO le triple effettivamente applicate.
        """
    ...
    def _create_statement_for_triple(self, subj, pred, obj):
        ...
        for uri, instances in self._instance_cache.items():
            for instance in instances:
                print(instance)
                if isinstance(instance, Statement):
                    subj = instance.get_has_subject()
                    pred = instance.get_has_predicate()
                    obj = instance.get_has_object()
                    print(f"S: {subj.has_identifier if subj else 'None'}")
                    print(f"P: {pred.has_identifier if pred else 'None'}")
                    print(f"O: {obj.has_identifier if obj else 'None'}")
                    print("---")
